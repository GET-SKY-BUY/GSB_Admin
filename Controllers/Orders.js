...
const Orders_Login = async ( req , res , next ) => {
    try {
        const Token = req.signedCookies.ORDER_TOKEN;
        if(Token){
        ...
        let Search = await Assistants.findOne({ Email: req.body.Email , Employee_Type: "Order Assistant" });
        if(!Search){
            return res.status(400).json({
                Status: "Failed",
        ...
        let Status = await Send_Mail({
            from: "Order assistant - OTP" + "<" + process.env.MAIL_ID + ">",
            to: req.body.Email,
            subject: "OTP Verification - Order assistant",
        ...
        await Search.save().then(() => {
            res.cookie("ORDER_OTP", NewToken, Cookie_Options_OTP);
...
const Order_Login_OTP = async ( req , res , next ) => {
    try {
        ...
        const VerifyOTP_TOKEN = req.signedCookies.ORDER_OTP;
        if(!VerifyOTP_TOKEN){
            return res.status(401).json({
                Status: "Failed",
        ...
        let Status = await Send_Mail({
            from: "Order Assistant - Login" + "<" + process.env.MAIL_ID + ">",
            to: Search.Email,
            subject: "Order Assistant - Logged in",
        ...
        await Search.save().then(c => {
            res.cookie("ORDER_TOKEN", NewToken, Cookie_Options_Final);
            res.clearCookie("ORDER_OTP",{
                domain: process.env.PROJECT_DOMAIN,
                path: "/",
                httpOnly: true,
                secure: process.env.NODE_ENV === 'production',
