...
const Delivery_Login = async ( req , res , next ) => {
    try {
        ...
        let Search = await Assistants.findOne({ Email: req.body.Email , Employee_Type: "Delivery Assistant" });
        if(!Search){
            return res.status(400).json({
                Status: "Failed",
        ...
        let Status = await Send_Mail({
            from: "Delivery assistant - OTP" + "<" + process.env.MAIL_ID + ">",
            to: req.body.Email,
            subject: "OTP Verification - Delivery assistant",
        ...
        await Search.save().then(() => {
            res.cookie("DELIVERY_OTP", NewToken, Cookie_Options_OTP);
...
const Delivery_Login_OTP = async ( req , res , next ) => {
    try {
        ...
        const VerifyOTP_TOKEN = req.signedCookies.DELIVERY_OTP;
        if(!VerifyOTP_TOKEN){
            return res.status(401).json({
                Status: "Failed",
        ...
        let Status = await Send_Mail({
            from: "Delivery Assistant - Login" + "<" + process.env.MAIL_ID + ">",
            to: Search.Email,
            subject: "Delivery Assistant - Logged in",
        ...
        await Search.save().then(c => {
            res.cookie("DELIVERY_TOKEN", NewToken, Cookie_Options_Final);
            res.clearCookie("DELIVERY_OTP",{
                domain: process.env.PROJECT_DOMAIN,
                path: "/",
                httpOnly: true,
                secure: process.env.NODE_ENV === 'production',
